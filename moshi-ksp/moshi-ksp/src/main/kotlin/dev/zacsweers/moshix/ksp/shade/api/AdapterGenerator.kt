...
import dev.zacsweers.moshix.ksp.shade.api.FromJsonComponent.ParameterOnly
import dev.zacsweers.moshix.ksp.shade.api.FromJsonComponent.ParameterProperty
import dev.zacsweers.moshix.ksp.shade.api.FromJsonComponent.PropertyOnly
...
internal class AdapterGenerator(
  ...
  private val generateBytecode: Boolean = false
) {
  ...
  private val constructorProperty by lazy(NONE) {
  ...
  fun prepare(typeHook: (TypeSpec) -> TypeSpec = { it }): PreparedAdapter {
    ...
    val (type, bridgeType) = generateType()
    val generatedAdapter = typeHook(type)
    result.addType(generatedAdapter)
    val fileSpec = result.build()
    return PreparedAdapter(fileSpec, generatedAdapter.createProguardRule(), bridgeType)
  }
  ...
  private fun generateType(): Pair<TypeSpec, BridgeGenerator?> {
    ...
    val typeRenderer: TypeRenderer = object : TypeRenderer() {
      override fun renderTypeVariable(typeVariable: TypeVariableName): CodeBlock {
        ...
        check(index != -1) { "Unexpected type variable $typeVariable for type $adapterName" }
    ...
    val (fromJsonFun, bridgeGenerator) = generateFromJsonFun(result)
  ...
  private fun generateFromJsonFun(classBuilder: TypeSpec.Builder): Pair<FunSpec, BridgeGenerator?> {
...
internal data class PreparedAdapter(
  val spec: FileSpec,
  val proguardConfig: ProguardConfig?,
  val bridgeType: BridgeGenerator?
)
